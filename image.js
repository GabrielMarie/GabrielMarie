...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!lb || !lbImg) return; // page sans lightbox -> on n'initialise pas
  ...
  const showImage = () => { if (!currentImages.length) return; lbImg.src = nz(currentImages[currentIndex]); setIndicator(); };
  const open = () => { showImage(); lb.classList.remove("fade-out"); lb.style.display = "flex"; void lb.offsetWidth; lb.classList.add("fade-in"); };
  const close = () => { lb.classList.remove("fade-in"); lb.classList.add("fade-out"); setTimeout(() => { lb.style.display = "none"; }, 200); };

  function getListFrom(el) {
    // 1) close-ups explicites sur l'élément cliqué
    const attr = el.getAttribute("data-closeups");
    if (attr) return attr.split(",").map(nz).filter(Boolean);
    // 2) groupe de vignettes
  ...
  // Délégation: un seul listener pour tout le document
